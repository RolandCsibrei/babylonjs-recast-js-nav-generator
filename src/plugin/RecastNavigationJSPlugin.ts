 ...
 */
export class RecastNavigationJSPlugin implements INavigationEnginePluginV2 {
  /**
  ...
  createNavMesh(
    ...
    const config = RecastNavigationJSPlugin._CreateNavMeshConfig(parameters);
  ...
  getRandomPointAround(
    ...
    const ret = this.navMeshQuery.findRandomPointAroundCircle(
      position,
      maxRadius,
      options
    );
  ...
  private _convertNavPathPoints(
    ...
    if (navPath.success) {
      ...
      console.warn(
        "Unable to convert navigation path point, because navPath generation has faileds."
      );
    }
  ...
  private _computeSmoothPath(
    ...
    const halfExtents =
      options?.halfExtents ?? this.navMeshQuery.defaultQueryHalfExtents;
    ...
    // find polygon path
    const findPathResult = this.navMeshQuery.findPath(
      startRef,
      endRef,
      start,
      end,
    ...
    if (lastPoly !== endRef) {
      const lastPolyClosestPointResult = this.navMeshQuery.closestPointOnPoly(
        lastPoly,
        end
      );
    ...
    while (polys.length > 0 && smoothPath.length < maxSmoothPathPoints) {
      ...
      // Move
      const moveAlongSurface = this.navMeshQuery.moveAlongSurface(
        polys[0],
        iterPos,
        moveTarget,
      ...
      const polyHeightResult = this.navMeshQuery.getPolyHeight(
        polys[0],
        result
      );
      ...
      if (
        ...
        if (offMeshConnectionPolyEndPoints.success) {
          if (smoothPath.length < maxSmoothPathPoints) {
            ...
            const endPositionPolyHeight = this.navMeshQuery.getPolyHeight(
              polys[0],
              iterPos
            );
  ...
  private static _GetSteerTarget(
    ...
    while (ns < outPoints.length) {
      ...
      if (
        !RecastNavigationJSPlugin._InRange(posA, posB, minTargetDist, 1000.0)
      ) {
        break;
      }
  ...
  getDefaultQueryExtent(): Vector3 {
    return new Vector3(
      this.navMeshQuery.defaultQueryHalfExtents.x,
      this.navMeshQuery.defaultQueryHalfExtents.y,
      this.navMeshQuery.defaultQueryHalfExtents.z
    );
  }
